            ...
            <div class="content">
                <div class="row">  
                    ...
                    <div class=" col-sm-8 col-9 text-right m-b-3">
                        <a href="{{route('admin.roles.create')}}" class="btn btn btn-outline-primary btn-rounded float-right my-1"><i class="fa fa-plus "></i> Agregar Rol</a>
                        
                        <form method="GET" action="{{ route('admin.roles.index') }}" class="d-flex flex-wrap col-12 col-md-9 col-lg-6 justify-content-end m-b-3 ml-auto">
                            <div class="input-group">
                                ...
                                <input id="search-focus" type="text" name="buscar" class="form-control form-control-sm text-center" placeholder="Buscar por nombre de rol" value="{{ request('buscar') }}" />
                            </div>
                        </form>
                    ...
                    <div class="form-group row col-12 col-md-8">
                        ...
                        <div class="col-auto d-flex align-items-center pl-0">
                            <select id="perPageSelect" class="form-control w-auto">
                                <option value="5" {{ $roles->perPage() == 5 ? 'selected' : '' }}>5</option>
					...
					<div class="col-md-12">
						<div class="table-responsive">
							<table  class="table table-bordered table-striped custom-table">
								...
								<tbody>
                                    @php
                                         $contador = ($roles->currentPage() - 1) * $roles->perPage() + 1;
                                    @endphp
                                    @if($roles->isEmpty())
                                    <tr>
									...
									<tr class="table-success">
                                        ...
                                        <td class="col-md-1 text-center" style="width: 100px;">
                                            @can('Editar roles')
                                            <a class="btn btn-sm btn btn btn-outline-primary" href="{{route('admin.roles.edit',$role)}}">
                                                <i class="fa-solid fa-pencil"></i>
                                            </a>
                                            @endcan
                                            @can('Eliminar roles')
                                            <form action="{{route('admin.roles.destroy', $role)}}" class="formulario-eliminar" method="POST" style="display: inline-block;">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <div class="pagination-container d-flex justify-content-end">
                                @if ($roles->currentPage() > 1)
                                    <a href="{{ $roles->previousPageUrl() }}" class="btn btn-outline-primary">&laquo; Anterior</a>
                                @endif

                                @for ($i = 1; $i <= $roles->lastPage(); $i++)
                                    <a href="{{ $roles->url($i) }}" class="btn btn-outline-primary">{{ $i }}</a>
                                @endfor

                                @if ($roles->hasMorePages())
                                    <a href="{{ $roles->nextPageUrl() }}" class="btn btn-outline-primary">Siguiente &raquo;</a>
                                @endif
                            </div>
						</div>
					</div>
    ...
    <script>
            ...
            $(document).ready(function() {
                ...
                if (!dataTableIsInitialized) {
                    let dataTable = $('#rol-table').DataTable(dataTableOptions);
...
<!--<script>
    $(document).ready(function() {
        var placeholderText = " Buscar por nombre de rol";
    ...
    <script>
        document.getElementById('perPageSelect').addEventListener('change', function() {
            ...
            var url = "{{ route('admin.roles.index') }}?perPage=" + perPage;
...
<script>
    Swal.fire({
        title: "<span class='playfair-display-font'>No se puede eliminar el rol porque hay registros asociados en otra tabla.</span>",
        showClass: {
            popup: `
                animate__animated
                animate__fadeInUp
